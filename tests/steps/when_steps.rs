...
async fn request_home_page(world: &mut DashboardWorld) {
    let tab = world.ensure_browser();
    let title = tab.get_title().expect("Failed to get title");
    assert!(!title.is_empty(), "Page title is empty!");
    
    // Update body sync
    world.last_response_body = tab.get_content().expect("Failed to get content");
}

#[when(expr = "I inspect the {string}")]
async fn inspect_element(_world: &mut DashboardWorld, _element_name: String) {
    // No-op: The "Then" steps will do the inspection on the current body
}

#[given(expr = "I am viewing on a {string}")]
async fn view_on_device(_world: &mut DashboardWorld, _device: String) {
    // Mocking responsive view - in a real browser this would resize window.
}

#[when("the page renders")]
async fn page_renders(_world: &mut DashboardWorld) {
    // No-op
}
